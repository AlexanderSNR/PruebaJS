...
console.log('---- EXAMPLE 1 --- ', changeCompaniesAndUsers(companies));
export function changeCompaniesAndUsers(companies) {
  companies.forEach((company) => {
    company.name = company.name[0].toUpperCase() + (company.name.substring(1));
    company.users = changeNameAndLastName(company.users);
  });
...
function changeNameAndLastName(users) {
  users.forEach((user) => {
