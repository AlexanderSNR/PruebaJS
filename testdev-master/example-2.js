...
console.log('---- EXAMPLE 2 --- ', resolveExample(companies, false));

function resolveExample(companies, hasCar) {
  companies.forEach((company) => {
    company.users = getUsers(company.users, hasCar);
    company.usersLength = company.users.length;
  });
  return companies;
}

function getUsers(users, hascar) {
  const list = [];
  users.forEach((user) => {
    if (user.car == hascar) {
      list.push(user);
